...
-- Отсортировать строки по количеству покупок - сначала месяцы с нaименьшим кол-вом покупок, потом с наибольшим.
...
select
    *
from v_orders_cnt
where orders_cnt = (select max(orders_cnt) from v_orders_cnt);
...
-- 5. Руководство авиакомпании снизило цены на билеты рейсов LL4S1G8PQW, 0SE4S0HRRU и JQF04Q8I9G. Скидка на билет экономкласса (Economy) составила 15%, на билет бизнес-класса (Business) — 10%, 
-- а на билет комфорт-класса (PremiumEconomy) — 20%. Определите цену билета в каждом сегменте с учетом скидки.В выборке должны присутствовать три атрибута — id, trip_id, new_price.
